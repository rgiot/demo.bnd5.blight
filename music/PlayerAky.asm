;       Stabilized AKY music player - V1.1, for Amstrad CPC.
...
;       v1.1: BREAKING CHANGE: Previously generated songs (such as done with AT2) are NOT compatible (hardware sounds will sound broken).
;             - Corrected another bug if using hardware-only sound with odd envelope (thanks Zik).
;             - To remove hooks, define PLY_AKYst_REMOVE_HOOKS (PLY_AKYst_REMOVE_HOOKS = 1).
...
;       - SIZE: Hooks for external calls (play/init) are present by default. Define PLY_AKYst_REMOVE_HOOKS to remove them (PLY_AKYst_REMOVE_HOOKS = 1).
...
;       - SIZE: The header is only needed for players that want to load any song. Most of the time, you don't need it. Erase both the init code and
;               the header bytes in the song.
...
PLY_AKYst_DisarkForceNonReferenceDuring3_{disarkCounter}:
        ...
        IFNDEF PLY_AKYst_REMOVE_HOOKS
                assert PLY_AKYst_Start == $               ;Makes sure no byte has been added before the hooks.
...
PLY_AKYst_RRB_NIS_HardwareOnly_Loop:            ;104 cycles.
                ;ds PLY_AKYst_NOP_LongestInState - 104, 0         ;For all the IS/NIS subcodes to spend the same amount of time.
                ld d,19                     ;Waits for 182 - 104 = 80 cycles.
                dec d
                jr nz,$ - 1
                nop
